...
public class GuildMessageReceived extends ListenerAdapter {
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if (event.getGuild().getId().equals("809866573405093908")) { // DISCORD PULA
            System.out.println("Discord Pula");
        }

        if (event.getGuild().getId().equals("829434946086371359")) { // DISCORD MERCATO NERO
            System.out.println("Discord MN");
        }

        if (event.getGuild().getId().equals(MainStrangersLife.StrangerLifeID)) {
            System.out.println("Discord Stranger");
        }
        ...
        try {
            if (event.getGuild().getId().equals(MainBBBdB.dBbbdbID)) {
               if (event.getMessage().getAuthor().getId().equals(Ciccina)) {
